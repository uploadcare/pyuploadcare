...
from pyuploadcare.exceptions import UploadcareException, TimeoutError, UploadError
...
str_settings = ("pub_key", "secret", "api_version", "api_base", "upload_base")
bool_settings = ("verify_api_ssl", "verify_upload_ssl")
...
def ucare_argparser():
    ...
    subparser.add_argument(
        "--starting_point", help="a starting point for filtering files", action="store"
    )
    subparser.add_argument(
        "--ordering", help="specify the way the files should be sorted", action="store"
    )
    subparser.add_argument(
        "--limit", help="files to show", default=100, type=int_or_none
    )
    subparser.add_argument(
        "--request_limit", help="files per request", default=100, type=int_or_none
    )
    subparser.add_argument(
        "--stored",
    ...
    subparser.add_argument(
        "--starting_point", help="a starting point for filtering groups", action="store"
    )
    subparser.add_argument(
        "--ordering", help="specify the way the groups should be sorted", action="store"
    )
    subparser.add_argument(
        "--limit", help="group to show", default=100, type=int_or_none
    )
    subparser.add_argument(
        "--request_limit", help="groups per request", default=100, type=int_or_none
    )
    ...
    subparser.add_argument(
        ...
        dest="timeout",
        default=30,
        help="Set wait seconds file uploading from url." " Default value is 30 seconds",
    )
...
def main(arg_namespace=None, config_file_names=("~/.uploadcare", "uploadcare.ini")):
