...
from pyuploadcare.ucare_cli.utils import pprint, promt, int_or_none, bool_or_none, bar
...
def sync_files(arg_namespace):
    ...
    def _get(url, max_retry=3):
        ...
        try:
            ...
            pprint(("Can't download file: `{0}`. " "Origin error: {1}").format(url, e))
            return
        ...
        for f in files:
            ...
            if os.path.exists(local_filepath) and not arg_namespace.replace:
                pprint(
                    ...
                    "To override it use `--replace` option".format(local_filepath)
                )
                continue
...
def save_file_locally(fname, response, size):
    ...
    with open(fname, "wb") as lf:
        for chunk in bar(
            response.iter_content(chunk_size), ceil(size / float(chunk_size)), fname
        ):
            lf.write(chunk)
...
class TrackedFileList(FileList):
    ...
    def __iter__(self):
        ...
        return self.iter_urls(self.last_page_url or self.api_url(), self.limit)
...
def add_sync_files_parser(subparsers):
    ...
    subparser.add_argument(
        "--replace", help="replace exists files", default=False, action="store_true"
    )
    subparser.add_argument("--uuids", nargs="+", help="list of file's uuids for sync")
    subparser.add_argument(
        "--effects",
        help=(
