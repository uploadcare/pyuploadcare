...
@pytest.mark.vcr
def test_group_create_by_uuid_and_cdn_url(capsys):
    main(
        arg_namespace(
...
@pytest.mark.vcr
def test_groups_list_without_params(capsys):
...
@pytest.mark.vcr
def test_groups_list_with_params(capsys):
    main(
        arg_namespace(
            "list_groups "
...
@pytest.mark.vcr
def test_groups_list_empty_result(capsys):
    main(arg_namespace("list_groups --limit 0"))
    captured = capsys.readouterr()
